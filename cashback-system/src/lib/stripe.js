...
/**
 * Fluxo descontinuado. Novas assinaturas usam apenas checkout recorrente do Stripe.
 */
export async function createPixCheckoutSession(planId, merchantId, merchantEmail) {
  console.warn('createPixCheckoutSession foi chamado, mas o fluxo PIX/Boleto foi descontinuado.', {
    planId,
    merchantId,
    merchantEmail,
  });
  ...
  return {
    success: false,
    error: 'Fluxo PIX/Boleto descontinuado. Use o checkout recorrente do Stripe.',
  };
}

/**
 * Fluxo descontinuado. Mantido apenas para compatibilidade defensiva.
 */
export async function redirectToPixCheckout(planId, merchantId, merchantEmail) {
  return await createPixCheckoutSession(planId, merchantId, merchantEmail);
}
